<template>
  <div class='component play demo-infinite-container' ref="contents">
      ...
      <scroller lock-x scrollbar-y use-pullup :pullup-config="loadingStatus" :height="contentHeight" ref="scroller" @on-pullup-loading="calList" v-if="collectionList.totalCount">
          <div class="box2">
          <template v-for="(item, index) in listData">
            <div class='mullist-item' :key="item.musicId">
              <div class='mullist-icon' @click="addToPlayListAndPlay({data: item,list: playlist})">
                  <img :src="item.cover_url_middle" style='max-width:100%' @error="setErrorImg">
              </div>
              <div class='mullist-info' @click="addToPlayListAndPlay({data: item,list: playlist})">
              ...
              <div class='mullist-add' @click="collectionDel(item)">
                  <img class="add-icon" src="static/images/CollectionunSelected@2x.png">
              </div>
            </div>
          </template>
          </div>
        ...
        <div class="no-data" v-if="collectionListData.length === 0">暂无收藏歌单</div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        totalPage: null,
        totalCount: 0,
        playlist: listArray[1] // 列表类型——播放列表
      }
    },
    computed: {
      ...mapState({
    ...
    methods: {
      ...
      loadData () {
        let self = this
        let promise = this.collectionListGetData({page: this.page, count: this.count})
        this.loading = true
        promise.then((data) => {
          let newData = data.data
          if (newData.length === 0) return // 空数据
      ...
      countPage () {
        ...
        this.totalPage = Math.ceil(this.collectionList.totalCount / this.count)
        this.totalCount = this.collectionList.totalCount
      },
      loadDone () {
