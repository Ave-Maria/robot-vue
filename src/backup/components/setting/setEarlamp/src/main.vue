<template>
  ...
  <div class='component SetEarlamp' v-if="open" @click="closeSetting()">
      <div class="icon"></div>
      <div class="content">
        <div class="text">耳灯设置</div>
        <template v-for="option in earlampOption">
        <div :class="['option',{active:(option.num === earlamp)}]" @click.stop="checkSelect(option)">
          {{option.name}}
        </div>
        </template>
      </div>
