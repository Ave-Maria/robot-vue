<template>
  ...
  <div class='component SetPoweroff' v-if="open" @click="closeSetting()">
      <div class="icon"></div>
      <div class="content">
        <div class="text">定时关机</div>
        <template v-for="option in poweroffOption">
        <div :class="['option',{active:(option.num === poweroff)}]" @click.stop="checkSelect(option)">
          {{option.name}}
        </div>
        </template>
      </div>
